...
void log_msg(const char *format, ...)
{
    ...
    vfprintf(DD_DATA->logfile, format, ap);
}
...
void log_fuse_context(struct fuse_context *context)
{
    ...
    log_struct(((struct dd_state *)context->private_data), logfile, %08x, );
    //log_struct(((struct dd_state *)context->private_data), mountdir, %s, );
...
int log_fcall(char * func, int retstat, int min_ret)
{
    log_msg("    %s returned %d\n", func, retstat);
    if (retstat < min_ret)
        log_msg("    ERROR %s: failed\n", func);
    else
        log_msg("    SUCCESS %s: finished without problems\n", func);
...
void log_hash(unsigned char *buf, int hash_len)
{   
    int i;
    log_msg("\n    HASH Line: ");
