...
struct task_struct
{
	...
	struct task_struct *prev, *next;	/* Used to link the task struct in the 
										   ...
										   runqueue */
/* ---------------- Do NOT Touch END-------------- */
	unsigned long long time_taken;			/* When process takes the CPU */
	unsigned long long time_lost;			/* When process loses the CPU */
	unsigned long long last_burst;			/* time_taken - time_lost = time of last execution*/
	double exp_burst;				
	double last_exp_burst;		
	unsigned long long last_rq_entry_time;	/* When process was inserted to rq */
};
